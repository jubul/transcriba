...
YAML_TEMPLATE = """# transcriba — generated by `transcriba init` on {date}
# Full reference of every option: docs/CONFIGURATION.md
# ${{VAR}} values are read from the environment or from the .env file next to this YAML.

latency_profile: {latency}          # fast | balanced | quality  (see docs/LATENCY.md)

server:
  title: "{title}"
  host: 0.0.0.0
  port: {port}
  admin_token: ${{TRANSCRIBA_ADMIN_TOKEN}}   # panel and ingest; lives in .env
  public_url: ${{TRANSCRIBA_PUBLIC_URL:-}}   # e.g. https://subs.myevent.org (for QR codes); empty = the URL clients use
  data_dir: data
...
defaults:
  engine: {engine}
  source_language: {source_language}   # 'auto' detects the language of every sentence
  target_languages: [{targets}]
  glossary: [{glossary}]
...
def init(
    ...
    typer.echo("Para acceso desde fuera de la red local (y micrófono desde otras máquinas) hace falta HTTPS: ver docs/OPERATIONS.md")
